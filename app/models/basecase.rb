class Basecase < ActiveRecord::Base
    ...
    def parse_code_arb(input_string, id)
        
       # the difference between this funciton and the previous function is, that the rule  
       ...
       # this holds the rule body with words instead of numbers, this ensures the arbs can be found for each element in the rule
       rule_body_arb = ""
       ...
       rules_array.each do |rule|
         ...
         # here the rule_body is also held in the rule_body_arb to be used later for arb calculations
         rule_body_arb = rule_body
         ...
         # this is the rule_body split into many parts
         ...
         if max_array_length == 0
               ...
               # add arb code here
               # check if each item in rule_body_arb has an arb
               # -> find arb where name = EBIT for example for item
               # find all currencies in the arbs
               # -> array_of_currencies << add_each currency
               # find unique currencies
               # -> array_of_currencies.uniq
               # loop through unique currencies
               # -> array_of_currencies.each do |item|
               # for each unique currency, create a rule body composed of numbers
               # -> item[currency][i] << for each year sub into the formula and evaluate
               # convert the absolute item[currency] into a relative value
               # -> item[currency] = (item[currency]/eval(rule_body)).round(2)
               # -> put it into the arb tavle for rule_head   
         ...
         if max_array_length > 0
             ...
             strings_to_evaluate.each do |item|
                 ...
                 # convert the absolute item[currency] into a relative value
                 # -> item[currency] = (item[currency]/eval(rule_body)).round(2)
                 # -> put it into the arb table for rule_head
             end
